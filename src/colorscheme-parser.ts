...
export class ColorSchemeParser {
	...
	private parseLuaTheme(theme: Partial<ColorScheme>) {
		const colorMatches = this.content.matchAll(
			/colors\.([a-zA-Z0-9_]+)\s*=\s*["']([^"']+)["']/g,
		)
		for (const match of colorMatches) {
			theme.colors![match[1]] = match[2]
		}

		const hlMatches = this.content.matchAll(
			/hl\(["']([^"']+)["']\s*,\s*{([^}]+)}/g,
		)
